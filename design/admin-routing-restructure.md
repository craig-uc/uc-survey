...
- **Breadcrumb coverage extended to every `/admin/*` page** (2026-07-17): the dashboard page was previously the only one rendering `BreadcrumbBar`. `/admin/home` (`src/app/admin/home/page.tsx`) now renders a single active "Home" crumb; `/admin/surveys` (`src/app/admin/surveys/page.tsx`) renders Home / Surveys; `/admin/surveys/[surveyId]` (`src/app/admin/surveys/[surveyId]/page.tsx`) renders Home / Surveys / the survey id, added once the async `params` promise resolves (consistent with that page's existing loading behavior). No new component was introduced — this only wires the existing `BreadcrumbBar` (`src/components/layout/BreadcrumbBar.tsx`) into the remaining pages for consistent in-app navigation.
